...
proc NetSwapTCL::NetSwapTCLfunc {} {
    ...
    set logDir "//sng-psrvr06/Automation-Public/MiscFiles/SKILL_Logs/TCL_Logs/Net_Swap"
	...
	puts $fp "Used TCL Script  : Net Swap"
	...
	if {$lDesign != $lNullObj} {
		...
		for {set i 0} {$i < $objlength} {incr i} {
			...
			while {$pOffPageConnector!=$lNullObj} {
				...
				set InpNetName [string toupper $InpNetName]
				#puts $InpNetName
				foreach inpdt $UnqUpDtList {
					set inpdt [string toupper $inpdt]
		...
		set DelPropList [lsort -unique $DelPropList]
		...
		for {set k 0} {$k < $objlength} {incr k} {
			...
			set DelpOffPageConnectorsIter [$Delobj NewOffPageConnectorsIter $lStatus $::IterDefs_ALL]
			set DelpOffPageConnector [$DelpOffPageConnectorsIter NextOffPageConnector $lStatus]
			while {$DelpOffPageConnector!=$lNullObj} {
				set DelpartName [DboTclHelper_sMakeCString]
				$DelpOffPageConnector GetName $DelpartName
				set DelInpNetName [DboTclHelper_sGetConstCharPtr $DelpartName]
				set DelInpNetName [string toupper $DelInpNetName]
				foreach Delinpdt $DelPropList {
					set DelinpPrse1 [lindex [split $Delinpdt ","] 0] 
					set DelinpPrse2 [lindex [split $Delinpdt ","] 1]
					if {$DelinpPrse2 == $DelInpNetName } {
						OPage $DelClSchematicNameStr $DelUPPageName
						set DelpId [$DelpOffPageConnector GetId $lStatus]
							if {$DelinpPrse1 != $DelpId } {
								SelectObjectById $DelpId
								SetProperty {Name} {DELETE}
								UnSelectAll
							}
					}
				}
			...
			delete_DboPageOffPageConnectorsIter $DelpOffPageConnectorsIter
			ui::PMActivate $folderpath
		}
	}
}

NetSwapTCL::NetSwapTCLfunc
